...
async updateSeoPage(id: number, data: Partial<InsertSeoPage>): Promise<SeoPage | undefined> {
  ...
  async generateInvoiceFromTimesheet(timesheetId: number, generatedBy: number): Promise<Invoice> {
    ...
    const invoiceData: InsertInvoice = {
      ...
      hourlyRate: await convertINRToUSD(parseFloat(billing.hourlyRate.toString())),
  ...
  async generateInvoiceFromBiWeeklyTimesheet(biWeeklyTimesheetId: number, generatedBy: number): Promise<Invoice> {
    ...
    const invoiceData: InsertInvoice = {
      ...
      totalAmount: totalAmountUSD,
      currency: 'USD',
      currencyConversionRate: currencyData.average,
      sixMonthAverageRate: currencyData.average,
      amountINR: amountINR,
      gstRate: 18.0,
      gstAmount: gstAmount,
      ...
      dueDate: dueDate.toISOString().split('T')[0],
      generatedBy,
      notes: `Invoice generated for bi-weekly period ${timesheet.periodStartDate} to ${timesheet.periodEndDate}. Converted from INR ${amountINR} at rate ${currencyData.average}`
    };
