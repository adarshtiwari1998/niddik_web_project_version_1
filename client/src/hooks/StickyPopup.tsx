...
const StickyIcon = () => <div className="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center text-white text-2xl cursor-pointer">?</div>;
...
const StickyPopup: React.FC<StickyPopupProps> = () => {
    ...
    useEffect(() => {
        // Load stored content if there's a choice
        if (choice) {
    ...
    useEffect(() => {
        // Update arrow position when popup position changes
        updateArrowPosition();
    }, [popupPosition, isOpen]);

    useEffect(() => {
        // Show the arrow when the popup opens
        if (isOpen) {
            updateArrowPosition();
            setShowArrow(true);

            // Hide the arrow after a short delay (e.g., 3 seconds)
    ...
    const updatePopupPosition = () => {
        ...
        if (spaceAbove > popupHeight) {
            topPosition = iconRect.top - popupHeight - 10;
        } else {
            topPosition = iconRect.bottom + 10;
        }
        setPopupPosition({
            top: topPosition,
            left: leftOffset,
    ...
    const updateArrowPosition = () => {
        if (iconRef.current && popupRef.current) {
            ...
            // Calculate center of the icon and popup
