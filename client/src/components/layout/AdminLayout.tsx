...
import { User, List, Home, FileText, Settings, ChevronRight, LogOut, Shield, Loader2, CalendarClock, Menu, X, Mail, Globe, Clock } from "lucide-react";
        ...
        <div className={`${isCollapsed ? 'w-12 sm:w-16' : 'w-64 sm:w-[276px]'} fixed left-0 top-[72px] sm:top-[88px] h-[calc(100vh-72px)] sm:h-[calc(100vh-88px)] transition-all duration-300 ease-in-out z-40 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700`}>
          <div className="h-full overflow-y-auto">
            ...
            <nav className="p-1 sm:p-2">
              ...
              <Link href="/admin/whitepaper-downloads">
                <div className={`flex items-center px-2 sm:px-3 py-1.5 sm:py-2 mb-1 rounded-md transition-colors cursor-pointer ${
                  location === "/admin/whitepaper-downloads" 
                    ? "bg-primary/10 text-primary" 
                    : "hover:bg-gray-100 dark:hover:bg-gray-700"
                } ${isCollapsed ? 'justify-center' : ''}`}
                title={isCollapsed ? "Whitepaper Downloads" : ""}
                >
                  <FileText className="h-4 w-4 sm:h-5 sm:w-5" />
                  {!isCollapsed && (
                    <>
                      <span className="ml-2 sm:ml-3 text-sm sm:text-base">Whitepaper Downloads</span>
                      <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 ml-auto" />
                    </>
                  )}
                </div>
              ...
              <Link href="/admin/timesheets">
                <div className={`flex items-center px-2 sm:px-3 py-1.5 sm:py-2 mb-1 rounded-md transition-colors cursor-pointer ${
                  location === "/admin/timesheets" 
                    ? "bg-primary/10 text-primary" 
                    : "hover:bg-gray-100 dark:hover:bg-gray-700"
                } ${isCollapsed ? 'justify-center' : ''}`}
                title={isCollapsed ? "Timesheet Management" : ""}
                >
                  <Clock className="h-4 w-4 sm:h-5 sm:w-5" />
                  {!isCollapsed && (
                    <>
                      <span className="ml-2 sm:ml-3 text-sm sm:text-base">Timesheet Management</span>
                      <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 ml-auto" />
                    </>
                  )}
                </div>
